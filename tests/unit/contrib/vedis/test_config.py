...
def test_get_variable(monkeypatch, vedis_config) -> None:
    ...
    assert vedis_config.get_variable("key1") == "1"
    assert vedis_config.get_variable("key2") == "2"
...
def test_get_variables_list(monkeypatch, vedis_config) -> None:
    ...
    assert "TEST_KEY1" in vedis_config.get_variables_list()
    assert "TEST_KEY2" in vedis_config.get_variables_list()
    assert "TEZT_KEY1" not in vedis_config.get_variables_list()
    assert "TEZT_KEY2" not in vedis_config.get_variables_list()
