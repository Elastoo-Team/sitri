...
@pytest.mark.parametrize(
    "toml_config_obj", [pytest.lazy_fixture("toml_config"), pytest.lazy_fixture("toml_data_config")]
)
def test_metadata(toml_config_obj) -> None:
...
def test_get_by_other(toml_config_obj) -> None:
    ...
    assert toml_config_obj._get_by_path(
        "test/test_key2", separator=toml_config_obj.separator
